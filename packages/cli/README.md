...
The experimental `glossa ui .` command opens a full-screen state-machine UI. Press Enter to connect or disconnect, `c` to clear activity, `?` for help, and `q` to quit. The screen shows the worker-account authority warning before connection.
