...
The experimental `glossa ui .` command opens a small persistent command shell. Type `start`, `stop`, `status`, `devices`, `help`, `clear`, or `quit`; Tab completes commands. Starting a session prints the worker-account authority warning and each file-write or command-start event.
