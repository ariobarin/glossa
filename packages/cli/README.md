...
The experimental `glossa ui .` command opens a keyboard-driven launcher for exposing the workspace, checking status, or listing devices. Use the arrow keys and Enter; press `q` to return to the shell. Starting from the launcher shows the same shell-authority warning and activity output as `glossa start`.
