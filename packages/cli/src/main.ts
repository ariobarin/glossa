...
import { runSpotlightUi } from "./ui-spotlight.js";
...
Opens an experimental search-first action launcher for the current workspace.
Existing commands and bare startup keep their current behavior.`,
...
async function runInteractive(path: string | undefined, allowBroadRoot: boolean): Promise<void> {
  const root = await selectExposureRoot(path, allowBroadRoot);
  await runSpotlightUi({
    workspace: root,
    start: async () => {
      await authenticatedCredentials();
      await runManagedSession(root, loadRelayEndpoints(), allowBroadRoot);
    },
    status: async () => await showStatus(false),
    devices: async () => await showDevices(false),
    login: async () => {
