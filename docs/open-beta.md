...
To try the experimental full-screen UI instead, run `glossa ui .`. Press Enter to connect or disconnect, `c` to clear activity, `?` for help, and `q` to quit. The worker-account authority warning is visible before the workspace is exposed.
