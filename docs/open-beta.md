...
To try the experimental guided launcher instead, run `glossa ui .`. Use the arrow keys and Enter to choose an action, or press `q` to exit. Choosing **Expose workspace** retains the normal security warning, activity output, and Ctrl+C disconnect behavior.
