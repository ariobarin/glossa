...
To try the experimental command shell instead, run `glossa ui .`. Type `start`, `stop`, `status`, `devices`, `help`, `clear`, or `quit`; Tab completes commands. The shell keeps the security warning and local activity reporting visible while a session runs.
