...
The worker scans stdout and stderr incrementally while retaining bounded overlap across chunks. On a recognizable authentication-secret match it clears both captured streams, requests process-tree termination, wakes status waiters, and makes start, status, retained-output reads, or cancellation return only `restricted_data_blocked`. Termination does not undo effects that occurred before detection, and the scanner cannot prevent direct network exfiltration. Public MCP structured results omit worker-local lifecycle timestamps and elapsed duration because clients do not need them to manage a command. The relay may use an internal elapsed duration to add a brief quiet-running `get_command` text notice, without adding it to `structuredContent` or the public output schema. `cancel_command` terminates the process tree. Disconnecting the worker rejects new jobs and terminates all active commands. Command state, worker IDs, sequences, scan tails, default snapshots, and retained output ranges remain transient and are never persisted by the relay.
